...
async def help(ctx):
    ...
    embed.set_thumbnail(url= "https://www.usnews.com/dims4/USNEWS/4552c79/2147483647/thumbnail/640x420/quality/85/?url=http%3A%2F%2Fmedia.beam.usnews.com%2F1c%2F7f%2F495e5f284c5b935bfc7e55deb2f3%2Finvestinggrapharrowsup.jpg")
    embed.set_image(url= "https://cdn.shopify.com/s/files/1/2118/1625/products/000786a-6_2000x2000.png?v=1586266264")
    embed.add_field(name='!quote <STOCK_TICKER>', value = "Returns the stock's prices for the day (opening, low, high, closing, previous closing)\n" ,inline = False)
    embed.add_field(name='!chart <STOCK_TICKER> <TIME_RANGE>', value = "Returns candlestick chart for a given stock *(Time ranges are as follows:\n Weekly = WK, Half Year = HY, Yearly = Y, Five Year = 5Y)*\n" ,inline = False)
    embed.add_field(name='!news <STOCK_TICKER> <ARTICLE_NUMBER>', value = "Returns a news article for a given stock (normally only yields 200 articles)\n" ,inline = False)
    embed.add_field(name='!recommend <STOCK_TICKER>', value = "Returns analyst recommendations as a graph\n" ,inline = False)
    embed.add_field(name='!eps <STOCK_TICKER>', value = "Returns a company's quarterly earnings as a graph\n" ,inline = False)
